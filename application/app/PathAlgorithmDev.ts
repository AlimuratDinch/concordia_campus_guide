...
const buildGraph = (nodes: GraphNode[]) => {
  ...
  nodes.forEach((node) => {
    // Ensure node.adjacent is an array, default to empty if undefined or null
    graph[node.id] = node.adjacent || [];

    // Only loop through neighbors if node.adjacent is not empty
    if (node.adjacent) {
      node.adjacent.forEach((neighbor) => {
        // If the neighbor hasn't been added to the graph, initialize it as an empty array
        if (!graph[neighbor]) {
          graph[neighbor] = [];
        }
        // Add the current node as adjacent to the neighbor
        graph[neighbor].push(node.id);
      });
    }
  });
...
const bfs = (graph: Record<string, string[]>, start: string, target: string): string[] | null => {
  const queue: string[][] = [[start]]; // Stores paths, not just nodes
  const visited = new Set<string>(); // Tracks visited nodes to prevent re-exploration

  // While there are paths to explore
  while (queue.length > 0) {
    const path = queue.shift()!; // Get the next path to explore
    const node = path[path.length - 1]; // The last node in the current path
...
const path = bfs(graph, "964", "H9-escalators");
